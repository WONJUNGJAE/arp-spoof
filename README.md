...
## c.f) sender가 저한테 직접 보내는 패킷 즉 스푸핑된게 아닌 일반 패킷과 스푸핑된거를 구분하는 코드는 짜지 못했습니다 제가 스푸핑된 패킷을 확인하는걸로 2가지 조건  조건 1: eth->dmac == my_mac 과 조건 2: ip->sip == sender_ip 이걸로 잡았는데 문제는 이 2개 조건은 지금 스푸핑 된거랑 일반 패킷에서 둘다 똑같이 표현됩니다
